...
export class FreeSpinScenarioGenerator {
   ...
   * @param {number} [config.yellowChestMultiplierPerBomb] — не используется: жёлтый сундук накапливает мультипликаторы бомб (step.multiplier / v3.bombMultiplier)
  ...
  constructor(config) {
    ...
    this.yellowChestMultiplierPerBomb = config.yellowChestMultiplierPerBomb ?? this.v3.bombMultiplier ?? 5; // для совместимости; накопление идёт по множителю бомбы
  ...
  /**
   * Нарастание мультипликатора жёлтого сундука только по взрыву (bombType === 'explode'), не по бомбе-множителю.
   */
  _stepHasBombExplode(step) {
    return step?.bombType === 'explode' && !!step?.explodedPositions?.length;
  }

  /**
   * В бонусной игре при каждом взрыве бомбы (explode) начисляется +5 к мультипликатору жёлтого сундука.
   */
  _getBombMultiplierForStep(step) {
    return 5;
  }

  /**
   * Добавляет к шагу поля фриспина и при необходимости увеличивает yellowChestMultiplier на мультипликатор бомбы.
   * Накопление идёт только по взрыву (explode), не по бомбе-множителю (spin-win-bomb / cascade-win-bomb).
   * @param {Object} step — мутирует step
   * @param {number} freeSpinsRemaining
   * @param {number} roundIndex
   * @param {string[]} activeChests
   * @param {number} yellowChestMultiplier — текущее значение до этого шага (накопленная сумма по взрывам)
  ...
  _enrichStep(step, freeSpinsRemaining, roundIndex, activeChests, yellowChestMultiplier) {
    ...
    // Сначала записываем мульт «до» этого шага — для расчёта мини-тотала (минивин × (5+мульт сундука)); нарастание — после
    step.yellowChestMultiplier = hasYellow ? yellowChestMultiplier : undefined;
    if (hasYellow && this._stepHasBombExplode(step)) {
      yellowChestMultiplier += this._getBombMultiplierForStep(step);
    }
  ...
  generateFreeSpinScenario(initialFreeSpins, maxFreeSpinRounds, activeChests) {
    ...
    this.meta = {
      ...
      isBonus: true,
      activeChests: chests,
      yellowChestMultiplierPerBomb: this.yellowChestMultiplierPerBomb  // множитель за одну бомбу (накопление в шагах — сумма мультипликаторов бомб)
    };
