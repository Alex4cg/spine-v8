...
/**
 * Установить сундуки в уровень 3 (открытый, бонусная игра) без проигрыша boom.
 * Используется при загрузке бонусного сценария: activeChests в bonus-init означает,
 * что эти сундуки уже на 3 уровне в начале бонусной игры.
 * @param {{ red?: spine.Spine, blue?: spine.Spine, yellow?: spine.Spine }} chests
 * @param {string[]} chestColors - массив цветов из activeChests (например ['yellow'])
 */
export function setChestsToBonusLevel(chests, chestColors) {
  if (!chests || !Array.isArray(chestColors)) return;
  const level3IdleAnim = getAnimationName(3, 'idle');
  for (const color of chestColors) {
    if (!CHEST_SKINS.includes(color)) continue;
  ...
  if (chestColors.length > 0) {
    console.log('[ChestManager] Сундуки установлены на уровень 3 (бонус):', chestColors.join(', '));
  }
}
...
export function playJumpOnAll(chests) {
  playBombBoomReaction(chests, { isBonus: false, activeChests: [] });
}

/**
 * Реакция сундуков на взрыв бомбы (ивенту "boom").
 * Правило только для жёлтого в бонусной игре: если жёлтый активен (activeChests включает 'yellow'),
 * он реагирует анимацией 03_boom (затем 03_idle). Остальные сундуки и жёлтый вне этого случая — XX_jump.
 *
 * @param {{ red?: spine.Spine, blue?: spine.Spine, yellow?: spine.Spine }} chests
 * @param {{ isBonus: boolean, activeChests: string[] }} options - isBonus и activeChests из сценария
 */
export function playBombBoomReaction(chests, options = {}) {
  if (!chests) return;
  const isBonus = !!options.isBonus;
  const activeChests = Array.isArray(options.activeChests) ? options.activeChests : [];
  const yellowUsesBoom = isBonus && activeChests.includes('yellow');

  for (const color of CHEST_SKINS) {
    const chest = chests[color];
    if (!chest || chest.destroyed) continue;

    if (color === 'yellow' && yellowUsesBoom) {
      const boomAnim = getAnimationName(3, 'boom');
      const idleAnim = getAnimationName(3, 'idle');
      if (hasAnimation(chest, boomAnim)) {
        const entry = chest.state.setAnimation(0, boomAnim, false);
        if (entry) {
          entry.listener = {
