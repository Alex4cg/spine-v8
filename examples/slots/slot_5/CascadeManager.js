...
// Дефолтный множитель бомбы, если в шаге сценария не прописан (актуальное состояние мульта в момент начисления)
const DEFAULT_BOMB_MULTIPLIER = 5;
// Прирост мульта жёлтого сундука за один взрыв (explode); в сценарии шаг хранит мульт «до» взрыва, нарастание — по событию boom
const YELLOW_CHEST_MULTIPLIER_PER_EXPLODE = 5;
...
export class CascadeManager {
  constructor(config) {
    ...
    /** Мультипликатор жёлтого сундука для UI: обновляется только по событию взрыва бомбы (onBombBoomEvent), не при спине/переходе шага */
    this.displayYellowChestMultiplier = 0;
    ...
    Symbol.onBombBoomEvent = () => {
      ...
      if (this.isBonusFromScenario) requestAnimationFrame(() => {
        this.displayYellowChestMultiplier += YELLOW_CHEST_MULTIPLIER_PER_EXPLODE;
  ...
  async cascade() {
    ...
    // Сброс Total Win только в обычной игре; в бонусной (фриспины) тотал копится весь бонус
    if (!this.isBonusFromScenario) {
      this.resetTotalWin();
      if (this.totalWinDisplay) this.totalWinDisplay.hide();
    }
  ...
  async _handleScenarioWinCheck(winningSymbols) {
    ...
    // Одна проверка: есть анимация взрыва = есть начисление с множителем (по event или по полям шага)
    const hasBomb = !!(winStep?.event === 'spin-win-bomb' || winStep?.event === 'cascade-win-bomb' ||
      (winStep?.bombType === 'multiplier' && winStep?.bombPosition));
    ...
    if (winningSymbols.length > 0) {
      ...
      // Обновление Total Win при выигрыше. Актуальный мульт в момент начисления — из шага или дефолт при hasBomb.
      if (winStep) {
        const useDefaultBombMult = hasBomb && (winStep.multiplier == null || winStep.multiplier === 0);
        const bombMult = useDefaultBombMult ? DEFAULT_BOMB_MULTIPLIER : (winStep.multiplier || 1);
        // Если мульт не в сценарии — totalPayout приходит как база; иначе в сценарии totalPayout уже = base * multiplier
        const basePayout = useDefaultBombMult ? (winStep.totalPayout || 0) : (bombMult !== 1 ? (winStep.totalPayout || 0) / bombMult : (winStep.totalPayout || 0));
        ...
        const effectiveMult = hasYellowBonus ? (this.displayYellowChestMultiplier ?? 0) + bombMult : bombMult;
  ...
  loadScenario(scenarioJson) {
    ...
    if (isBonusInit) {
      ...
      this.displayYellowChestMultiplier = 0;
      if (this.onBonusModeChange) this.onBonusModeChange(true, this.activeChestsForBonus, freeSpins, this.displayYellowChestMultiplier);
  ...
  _emitBonusUIUpdate() {
    ...
    // Мульт сундука — только из значения, обновлённого по взрыву бомбы (не из текущего шага сценария)
    const yellowMult = this.displayYellowChestMultiplier ?? 0;
