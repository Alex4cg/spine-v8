...
export class FontManager {
  ...
  async loadFonts() {
    ...
    while (attempts < maxAttempts) {
      const ok = document.fonts.check('400 56px "The Bomb Sound"') && document.fonts.check('400 65px "GAMERIA"') && document.fonts.check('700 35px "Roboto Condensed"');
      if (ok) {
        const activator = document.createElement('div');
        activator.style.fontFamily = '"The Bomb Sound", "GAMERIA", "Roboto Condensed"';
