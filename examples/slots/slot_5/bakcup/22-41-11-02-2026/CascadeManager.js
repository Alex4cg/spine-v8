...
export class CascadeManager {
  constructor(config) {
    ...
    // По ивенту "boom" в анимации бомбы: остановка scale-down, баунс масштаба в 1, тряска поля + boom_disappearance + опциональный config.onBombBoom
  ...
  async _handleScenarioWinCheck(winningSymbols) {
    ...
    if (winningSymbols.length > 0) {
      ...
      if (this.miniWinManager && isMultiClusterNoBomb) {
        this.miniWinManager.showMiniWinsAndTotalForMultiCluster(winStep); // фон, исчезновение накладывается на досыпание
        await this._removeWinningSymbolsWithWinAnimation(winningSymbols, {});
        // clear() не вызываем — минивин/тотал завершатся сами
      } else if (this.miniWinManager && hasBomb) {
        this.miniWinManager.showMiniWinsAndTotalWithBomb(winStep, 2000); // фон
        await this._removeWinningSymbolsWithWinAnimation(winningSymbols, { useBombScenario: true });
        // clear() не вызываем — минивин/тотал завершатся сами
      } else {
    ...
    if (nextStep.event === 'cascade-bomb' && nextStep.bombType === 'explode') {
      ...
      // Если после взрыва есть каскад, продолжаем обработку
      if (stepAfterBomb.event === 'cascade' || stepAfterBomb.event === 'cascade-win' || stepAfterBomb.event === 'cascade-win-bomb') {
    ...
    setTimeout(async () => {
      ...
      if (this.currentScenario) {
        ...
        if (nextStep.event === 'cascade-bomb' && nextStep.bombType === 'explode') {
          ...
          this.currentScenarioStep = stepAfterBombIndex;
          const stepAfterBomb = this.currentScenario[stepAfterBombIndex];
          // Если после взрыва есть выигрыш, обрабатываем его
          ...
          } else if (stepAfterBomb.event === 'cascade') {
            await this.processCascade();
          } else {
            this._setIdle();
            this.winCheckProcessed = false;
      ...
      if (winningSymbols.length > 0) {
        ...
        if (this.scenarioMode) {
          ...
          // Проверяем условие для проигрывания hit анимации скаттеров:
          // Спин с выигрышем (текущий шаг - cascade, следующий - cascade без выигрыша)
          const curStep = this.getCurrentScenarioStep();
          const nextStepIndex = (this.currentScenarioStep + 1) % this.currentScenario.length;
          const nextStep = this.currentScenario[nextStepIndex];
          if (curStep && curStep.event === 'cascade' && nextStep && 
              (nextStep.event === 'cascade' || nextStep.event === 'spin') && 
              (!nextStep.winningGroups || nextStep.winningGroups.length === 0)) {
  ...
  async _handleBombExplode(step) {
    ...
    for (const pos of step.explodedPositions) {
      ...
      const row = pos.row;
      if (col >= 0 && col < this.GRID_COLS && row >= 0 && row < this.GRID_ROWS) {
        // Проверяем groupMarkup: если значение -2, это скаттер под взрывом (не удаляем)
        if (step.groupMarkup && step.groupMarkup[row] && step.groupMarkup[row][col] === -2) {
          continue; // Пропускаем скаттер
        }
