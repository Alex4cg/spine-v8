...
export class ScenarioGeneratorV3 {
  constructor(config) {
    ...
    /** Параметры скаттеров */
    this.scatterSpinChance = config.scatterSpinChance || 0.03;  // Шанс появления скаттера на спине
    this.scatterRefillChance = config.scatterRefillChance || 0.01;  // Шанс при досыпке после спина
    /** Шанс появления скаттера при досыпке внутри каскада (если не задан — используется scatterRefillChance) */
  ...
  fillEmptySpots(grid, emptySpots, getNextSymbol, bombAlreadyUsed, isCascadeRefill = false) {
    ...
    for (let i = 0; i < emptySpots.length; i++) {
      ...
      if (!bombAlreadyUsed && !hasBombAfterCompact && Math.random() < bombChance) {
        ...
        // С шансом scatterChance размещаем скаттер случайного типа
        const scatterTypes = [this.SCATTER_BLUE, this.SCATTER_GOLD, this.SCATTER_RED];
  ...
  createEvent(grid, eventType, winningGroups, bombPosition, bombAlreadyUsed) {
    ...
    // Если есть бомба без выигрыша (взрыв), получаем explodedPositions для пометки скаттеров
    ...
    // Создаём groupMarkup с учётом explodedPositions для пометки скаттеров
    groupMarkup = this.createGroupMarkup(grid, winningGroups, explodedPositions);
