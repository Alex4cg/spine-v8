  ...
  async _onAllLanded() {
    ...
    setTimeout(async () => {
      ...
      if (nextStep && (nextStep.event === 'cascade' || nextStep.event === 'cascade-win' || nextStep.event === 'cascade-win-bomb')) {
        ...
        const curStep = this.getCurrentScenarioStep();
        const stepAfterNextIndex = (nextStepIndex + 1) % this.currentScenario.length;
        const stepAfterNext = this.currentScenario[stepAfterNextIndex];
        if (curStep && curStep.event === 'cascade' && stepAfterNext &&
            (stepAfterNext.event === 'cascade' || stepAfterNext.event === 'spin') &&
            (!stepAfterNext.winningGroups || stepAfterNext.winningGroups.length === 0)) {
      ...
      if (currentStep && (currentStep.event === 'cascade' || currentStep.event === 'spin' || currentStep.event === 'spin-win-bomb' ||
        ...
        if (currentStep.event === 'spin') {
          this._playScatterHitAnimations();
        } else if (currentStep.event === 'cascade' && nextStep && nextStep.event === 'spin') {
          this._playScatterHitAnimations();
        }
      } else {
