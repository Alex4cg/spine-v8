  ...
  static initSpinButton(spinButton, grid) {
    ...
    spinButton.onclick = async (e) => {
      ...
      if (grid.scenarioMode && grid.currentScenario) {
        ...
        if (currentStep && (currentStep.event === 'spin' || currentStep.event === 'cascade')) {
          // Мы на шаге ожидания спина — по нажатию переходим на следующий шаг и крутим спин
          const nextIndex = grid.currentScenarioStep + 1;
          if (nextIndex < grid.currentScenario.length) {
        ...
        } else if (!currentStep || (currentStep.event !== 'spin' && currentStep.event !== 'spin-win')) {
