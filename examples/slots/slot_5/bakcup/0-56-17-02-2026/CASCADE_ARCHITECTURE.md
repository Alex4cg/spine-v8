...
- Шаги сценария: `event` — `bonus-init` | `spin` | `spin-win` | `spin-bomb` | `cascade` | `cascade-win` | `cascade-bomb` | `cascade-win-bomb`. Выигрышные символы берутся из текущего шага: `winningGroups` + `groupMarkup` (см. `_getWinningSymbolsFromScenario()`).
- **bonus-init:** служебный первый шаг фриспин-сценария. При загрузке сценария, у которого шаг 0 — `event: "bonus-init"` и `isBonus: true`, игра переходит в бонусный режим без транзишена: применяются `activeChests`, `yellowChestMultiplierPerBomb`, добавляется бонусный фон, показываются счётчик фриспинов и (при активном жёлтом сундуке) счётчик мультипликатора. Первый игровой спин берётся со второго шага (индекс 1). При загрузке обычного сценария бонусное состояние сбрасывается.
- **События с бомбами:**
...
- **intrigueHighlightContainer:** контейнер для Spine-подсветки рила в режиме интриги (zIndex 110). Находится поверх символов, под bombWinOverlayContainer. Содержит один экземпляр Spine из `spine/intrigue`, позиционируется по центру колонки, которая в данный момент заполняется. Показывается только при интриге, проявляется через прозрачность при переключении колонок.
- **gameFieldContainer:** основной контейнер игрового поля. Содержит поле, символы, маску, intrigueHighlightContainer, bombWinOverlayContainer. Является `shakeContainer` для трансформаций при взрыве.
...
- **ScatterFlySystem:** система перелётов монет от скаттеров к целям. **Регулярная игра:** цель — сундуки (по цвету скаттера), по прилёту вызывается `onCoinArrival`. **Бонусная игра (фриспины):** цель — индикатор фриспинов (`setFreeSpinsTarget`), по прилёту в `onFlightComplete` передаётся `'freespins'`, вызывается `grid.incrementFreeSpins()`. Активируется при событии `shot` от скаттера (после `_playScatterHitAnimations()`). См. [SCATTER_FLIGHT_SYSTEM.md](SCATTER_FLIGHT_SYSTEM.md).
...
- **Режим интриги:** определяется **по сценарию** до начала падения (без отслеживания приземлений). Для колонки `col` интрига активна, если сумма скаттеров (textureIndex 9, 10, 11) в колонках 0..col-1 ≥ 3. **Обычное насыпание:** ROW_DELAY, COLUMN_DELAY. **Режим интриги:** rowDelay ×1.5; задержка между колонками = сумма пауз между символами колонки = `(GRID_ROWS - 1) * rowDelay`. **Основной спин** (`_startCascadeFall`): `_intrigueForColumn` заполняется по `step.grid` через `_shouldColumnHaveIntrigue(step, col)`. **Каскад-досыпка** (`startNewSymbolsFall`): при `step.scatterCount ≥ 4` интрига отключена; иначе — та же проверка. Методы: `_countScattersInColumn(step, colIndex)`, `_shouldColumnHaveIntrigue(step, col)`, `_getEffectiveDelays(col)`.

- **Подсветка рила в режиме интриги:** Spine-анимация из `spine/intrigue` (одна анимация `loop`) подсвечивает колонку, которая в данный момент заполняется. Контейнер `intrigueHighlightContainer` (zIndex 110) в `gameFieldContainer`. Подсветка показывается только на колонках с интригой. **Первое появление:** задержка 0.5 сек (`INTRIGUE_FIRST_SHOW_DELAY`). **При каждом переключении** на новую колонку контейнер проявляется через прозрачность (alpha 0 → 1 за ~150 ms). Конфиг: `intrigueSpineSkeletonAlias`, `intrigueSpineAtlasAlias`, `intrigueHighlightContainer`. Методы: `_showIntrigueHighlight(col)`, `_hideIntrigueHighlight()`.
