...
**Когда:** В момент нажатия кнопки спин — в начале метода `cascade()` ([CascadeManager.js](slots/slot_5/CascadeManager.js), строка ~510).

**Что делать:**
- Вызвать `resetTotalWin()` — обнулить накопленную сумму.
- Скрыть отображение Total Win (`totalWinDisplay.hide()`) — перестать показывать последнее значение.
