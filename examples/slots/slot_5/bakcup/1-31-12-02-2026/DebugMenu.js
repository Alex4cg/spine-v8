...
export function createDebugMenu(app, elements, config) {
  ...
  if (app && elements) {
    ...
    async function loadScenarioList() {
      ...
      // По умолчанию выбираем и загружаем scenario_defolt.json
      const defaultFileName = scenarioList.includes('scenario_defolt.json') ? 'scenario_defolt.json' : scenarioList[0];
      if (scenarioList.length > 0 && defaultFileName) {
        scenarioSelect.value = defaultFileName;
        const response = await fetch(`./scenario/${defaultFileName}`);
        const scenarioData = await response.json();
        scenarioInfo.textContent = `Ready to start: ${scenarioData.length} steps`;
        scenarioInfo.style.color = '#4CAF50';
