...
export class CascadeManager {
  constructor(config) {
    ...
    // По ивенту "boom" в анимации бомбы: звук взрыва, остановка scale-down, баунс масштаба в 1, тряска поля + boom_disappearance + scatter boom + опциональный config.onBombBoom
  ...
  async _handleScenarioWinCheck(winningSymbols) {
    ...
    if (winningSymbols.length > 0) {
      ...
      if (winStep) {
        // Берём готовые значения из сценария
        const basePayout = winStep.totalPayout || 0;
        const multiplier = winStep.multiplier || 1;
        
        // Вычисляем на клиенте: итоговая сумма шага с учётом бомбы
        const stepTotalPayout = basePayout * multiplier;
        
        // Вычисляем на клиенте: прибавляем к накопленной сумме за серию
