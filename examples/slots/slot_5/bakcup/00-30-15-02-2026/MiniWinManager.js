...
export class MiniWinManager {
   ...
   * @returns {Promise<void>}
   */
  async showMiniWinsAndTotalWithBomb(step, durationMs = 2000) {
    await new Promise(resolve => setTimeout(resolve, 1000));
    await this.showMiniTotalWithBomb(step, durationMs);
  }
  ...
  /**
   * Показывает мини тотал при бомбе: сначала множитель (например "10×5"), затем результат ("50").
   * Тайминги по MINI_WIN_DISPLAY.md: этап 1 — задержка 100, появление 233, видимый 800 мс; этап 2 — видимый 800 мс, исчезновение 300 мс.
   * @param {Object} step - Шаг сценария с totalPayout, multiplier
   * @param {number} [_durationMs] - не используется, тайминги заданы явно
   * @returns {Promise<void>}
   */
  async showMiniTotalWithBomb(step, _durationMs = 2000) {
    SoundManager.playWin();
    this._ensureContainer();

    const total = step?.totalPayout ?? 0;
    const mult = step?.multiplier ?? 1;
    const result = total * mult;
    const center = this._getGridCenter();

    // Этап 1: множитель — задержка 100, появление 233, видимый 800 мс, без исчезновения
    const stage1Duration = 100 + 233 + 800;

    const multText = `${Number(total).toFixed(2)}×${mult}`;
    ...
    const resultSprite = this.fontManager.createText('win_amount', this._formatPayout(result));
