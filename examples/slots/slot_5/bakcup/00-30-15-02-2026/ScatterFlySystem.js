...
export class ScatterFlySystem {
  ...
  startFlightFromScatter(symbol) {
    ...
    const chest = this.chests?.[chestColor];
    ...
    const endX = Number(chest.x) + this.config.chestOffsetX;
    const endY = Number(chest.y) + this.config.chestOffsetY;
    ...
    requestAnimationFrame(() => {
      this._createFlight(scatterType, chestColor, startX, startY, endX, endY);
    });
  }

  _createFlight(scatterType, chestColor, startX, startY, endX, endY) {
  ...
  _completeFlight(index, flight) {
    ...
    // Добавляем цвет сундука в массив активных (если еще не добавлен)
    if (chestColor && !this.activeChests.includes(chestColor)) {
    ...
    if (typeof this.config.onFlightComplete === "function") {
      try {
        this.config.onFlightComplete(chestColor);
      } catch (error) {
