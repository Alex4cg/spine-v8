...
export class FontManager {
  ...
  async loadFonts() {
    ...
    while (attempts < maxAttempts) {
      const ok = document.fonts.check('400 56px "The Bomb Sound"') && document.fonts.check('400 65px "GAMERIA"') && document.fonts.check('700 35px "Roboto Condensed"') && document.fonts.check('700 48px "Karantina"');
      if (ok) {
        const activator = document.createElement('div');
        activator.style.fontFamily = '"The Bomb Sound", "GAMERIA", "Roboto Condensed", "Karantina"';
  ...
  createText(styleName, text, overrides = {}) {
    ...
    const cacheKey = `fm_${styleName}_${text}_${fontSize}_${color}_${borderColor || ''}_${scaledBorderWidth}_${twoLayer || ''}_${layer1BorderColor || ''}_${scaledLayer1BorderWidth}_${letterSpacing || ''}_${shadowKey}_x${RENDER_SCALE}`;
