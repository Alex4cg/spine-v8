    ...
    this.elements = {
      'reelsContainer': {
        ...
        getRelatedElements: () => {
          ...
          // Винлайны получают сдвиг автоматически через syncWithReelsContainer и updatePosition
          // но не должны быть в relatedElements, так как позиционируются относительно центра поля
          if (this.slotMachine.winLineManager) {
            this.slotMachine.winLineManager.updatePosition(this.slotMachine);
          }
          // Обновляем позиции монеток при изменении позиции reelsContainer
          if (this.slotMachine.coinManager) {
            this.slotMachine.coinManager.updateAllCoinPositions();
          }
          return elements;
        }
      },
      ...
      'winline': {
        name: 'Winline Spine Container',
        getElement: () => {
          // Возвращаем контейнер винлайнов
          return this.slotMachine.winLineManager?.getContainer();
        },
        defaultValues: { 
          x: this.slotMachine.config?.spine?.winline?.position?.x || 960, 
          y: this.slotMachine.config?.spine?.winline?.position?.y || 540, 
          scale: this.slotMachine.config?.spine?.winline?.scale || 1, 
          zIndex: this.slotMachine.config?.spine?.winline?.zIndex || 180 
        }
      },
      'coinIndicatorsContainer': {
        name: 'Coin Indicators Container',
        getElement: () => {
          // Возвращаем контейнер всех индикаторов (на stage, отдельный контейнер)
          return this.slotMachine.coinIndicatorsContainer;
        },
        defaultValues: {
          x: 507, // Дефолтная позиция (настроено через дебаггер)
          y: 832, // Дефолтная позиция (настроено через дебаггер)
          scale: 1,
          zIndex: 104
        }
      }
