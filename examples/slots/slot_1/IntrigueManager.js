...
export class IntrigueManager {
  ...
  async showIntrigueAnimation(reels) {
    ...
    try {
      ...
      const reel2 = reels[2];
      if (!reel2) {
        console.warn('IntrigueManager: Cannot show intrigue animation - reel 2 not found');
        return;
      }
      ...
      if (loaded) {
        ...
        console.log(`IntrigueManager: Intrigue animation shown at (${intrigueX}, ${intrigueY})`);
      } else {
        console.error('IntrigueManager: Failed to load intrigue animation');
        this.intrigueAnimation = null;
      }
    } catch (error) {
      console.error('IntrigueManager: Error showing intrigue animation:', error);
      this.intrigueAnimation = null;
    }
  }
    ...
    if (this.intrigueAnimation) {
      ...
      const fadeOut = () => {
        ...
        if (progress < 1) {
          ...
          container.alpha = 0;
          console.log('IntrigueManager: Intrigue animation hidden');
        }
      };
   ...
   */
  async showIntrigueCoins(currentScenarioData) {
    ...
    console.log('IntrigueManager: Intrigue coins shown with anticipation animation');
  }
  ...
  async createIntrigueCoin(reelIndex, positionIndex, coinValue) {
    ...
    if (!loaded) {
      console.warn(`IntrigueManager: Failed to load intrigue coin Spine at ${key}`);
      return;
    }
    ...
    console.log(`IntrigueManager: Intrigue coin created at ${key} with value ${formattedValue}`);
  }
  ...
  hideIntrigueCoins() {
    ...
    // Очищаем объект
    this.intrigueCoins = {};
    console.log('IntrigueManager: Intrigue coins hidden and destroyed');
  }
  
  /**
   * Показывает отдельные экземпляры Spine коллекторов для интриги (играют anticipation или idle)
   */
  async showIntrigueCollectors(currentScenarioData) {
    ...
    console.log('IntrigueManager: Intrigue collectors shown with anticipation/idle animation');
  }
  ...
  async createIntrigueCollector(reelIndex, positionIndex, collectorValue) {
    ...
    if (!loaded) {
      console.warn(`IntrigueManager: Failed to load intrigue collector Spine at ${key}`);
      return;
    }
    ...
    console.log(`IntrigueManager: Intrigue collector created at ${key} with value ${formattedValue}`);
  }
  ...
  hideIntrigueCollectors() {
    ...
    // Очищаем объект
    this.intrigueCollectors = {};
    console.log('IntrigueManager: Intrigue collectors hidden and destroyed');
  }
}
