...
// Grand (Jg) доступен только в бонуске / mystery → в базовом сценарии его не генерируем.
const JACKPOT_COINS = ["Jm", "Jd", "Jr"]; // Mini / Midi / Major
...
function generateSymbolForRow(rowIndex, colIndex, options = {}) {
  ...
  if (rowIndex === 0 || rowIndex === 2) {
    const items = [
      ...
      // Sticky в верхнем/нижнем рядах делаем редкими
      ...STICKY_COINS.map((code) => ({ value: code, weight: 0.1 })),
  ...
  if (stickyInRow < 2) {
    STICKY_COINS.forEach((code) => {
      // В среднем ряду sticky тоже заметно реже, чем pot‑регулярные
 ...
 *
 * Sticky‑монеты наследуются от предыдущего спина:
 * - на вход можно передать prevGrid;
 * - все sticky из prevGrid остаются на своих местах;
 * - новые символы генерируются только в не‑sticky ячейках.
 *
 * @param {{ noBonus?: boolean, prevGrid?: string[][] }} [options]
 * @returns {string[][]}
 */
function generateRandomGrid3x3(options = {}) {
  ...
  for (let row = 0; row < ROWS; row += 1) {
    ...
    for (let col = 0; col < COLS; col += 1) {
      // Если в предыдущем спине здесь была sticky‑монета — наследуем её.
      const prevSymbol =
        options.prevGrid &&
        options.prevGrid[row] &&
        typeof options.prevGrid[row][col] === "string"
          ? options.prevGrid[row][col]
          : null;

      if (prevSymbol && isStickyCode(prevSymbol)) {
        rowData.push(prevSymbol);
        if (row === 1) {
          stickyInRow += 1;
        }
        continue;
...
export function generateInitialScenario(spinCount = DEFAULT_SPIN_COUNT, options = {}) {
  ...
  for (let i = 0; i < spinCount; i += 1) {
    const grid = generateRandomGrid3x3({ ...options, prevGrid });
