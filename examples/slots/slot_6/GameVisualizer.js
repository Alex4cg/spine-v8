...
export class GameVisualizer {
  constructor(config = {}) {
    ...
    // Контейнеры для дебага
    this.stackContainer = new PIXI.Container(); // Контейнер для стека ожидающих символов
    this.fieldContainer = new PIXI.Container(); // Контейнер для игрового поля
  ...
  init() {
    ...
    this.container.addChild(this.stackContainer);
  ...
  initSelectionFrame() {
    if (this.selectionFrame) {
      this.fieldContainer.removeChild(this.selectionFrame);
    }
    this.selectionFrame = new PIXI.Graphics();
    this.selectionFrame.visible = false;
    this.fieldContainer.addChild(this.selectionFrame);
  }
    ...
    for (let row = 0; row < this.STACK_ROWS; row++) {
      for (let col = 0; col < this.STACK_COLS; col++) {
        if (this.stackSprites[row] && this.stackSprites[row][col]) {
          this.stackContainer.removeChild(this.stackSprites[row][col]);
  ...
  clearField() {
    for (let i = 0; i < this.FIELD_SIZE; i++) {
      if (this.fieldSprites[i]) {
        this.fieldContainer.removeChild(this.fieldSprites[i]);
  ...
  renderStack(stack) {
    ...
    for (let row = 0; row < this.STACK_ROWS; row++) {
      for (let col = 0; col < this.STACK_COLS; col++) {
        ...
        // Позиция относительно контейнера стека
        const x = col * (this.SYMBOL_SIZE + this.SYMBOL_SPACING);
        const y = row * (this.SYMBOL_SIZE + this.SYMBOL_SPACING);
        
        const sprite = this.createSymbolSprite(symbolId, x, y);
        this.stackSprites[row][col] = sprite;
        this.stackContainer.addChild(sprite);
      }
    }
  }
  ...
  renderField(field) {
    ...
    for (let i = 0; i < this.FIELD_SIZE; i++) {
      ...
      // Позиция относительно контейнера поля
      const x = i * (this.SYMBOL_SIZE + this.SYMBOL_SPACING);
      const y = 0;
  ...
  renderSelectionFrame(index) {
    ...
    // Позиция относительно контейнера поля
    const x = index * (this.SYMBOL_SIZE + this.SYMBOL_SPACING);
    const y = 0;
  ...
  renderMultipliers(multipliers) {
    ...
    // Позиция относительно контейнера (абсолютная позиция на экране)
    const startX = CONFIG.POSITIONS.MULTIPLIERS_X;
